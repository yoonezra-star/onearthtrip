...
const sizeCache = new Map<string, ImageSize | undefined>();
...
function getPngSize(buffer: Buffer): ImageSize | undefined {
  if (
    buffer.length < 24 ||
    buffer[0] !== 0x89 ||
...
function getGifSize(buffer: Buffer): ImageSize | undefined {
  if (buffer.length < 10 || !buffer.toString("ascii", 0, 6).startsWith("GIF8")) {
    return undefined;
  }
...
function getWebpSize(buffer: Buffer): ImageSize | undefined {
  if (
    buffer.length < 30 ||
...
function getJpegSize(buffer: Buffer): ImageSize | undefined {
...
export function getHeroImageAlt(body: string, heroImage: string, fallback: string) {
  const normalizedHero = normalizeAssetPath(heroImage);

  for (const match of body.matchAll(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+["'][^)]*["'])?\)/g)) {
    const alt = match[1].trim();
    const src = normalizeAssetPath(match[2]);
    if (src === normalizedHero && alt) return alt;
  }
  ...
  for (const match of body.matchAll(/<img\b[^>]*>/gi)) {
    ...
    if (src && normalizeAssetPath(src) === normalizedHero && alt) return alt;
  }

  return `${fallback} 대표 이미지`;
}
